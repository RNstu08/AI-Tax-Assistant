...
def run_retention_cleanup(store: ProfileStore, apply: bool = False) -> dict:
    ...
    for user_id in store.get_all_user_ids():
        ...
        user_summary = {"deleted_attachments": 0, "deleted_evidence": 0}

        if days := retention_days.get("attachments_days"):
            if int(days) > 0:
                cutoff = now_ms - (int(days) * day_ms)
                # This part is a dry run if apply is False
                count = (
                    store.delete_attachments_older_than(user_id, cutoff)
                    if apply
                    else len(store.list_attachments_older_than(user_id, cutoff))
                )
                if apply:
        ...
        if days := retention_days.get("evidence_days"):
            if int(days) > 0:
                cutoff = now_ms - (int(days) * day_ms)
                count = (
                    store.delete_evidence_older_than(user_id, cutoff)
                    if apply
                    else len(store.list_evidence_older_than(user_id, cutoff))
                )
                if apply:
